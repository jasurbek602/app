...
function serializeMessage(msg, chatKey) {
  const base = {
    id: msg._id,
    chat: msg.chat,
    sender: msg.sender,
  ...
  if (base.isDeleted) return base; // o'chirilgan xabarning matni/mediasi hech qachon qaytarilmaydi
  if (msg.type === "text") {
    base.text = decryptMessage(msg.cipherText, chatKey);
  } else {
    base.media = {
      url: `/api/media/${msg.media.fileId}`,
...
router.post("/:chatId/text", requireAuth, async (req, res) => {
  try {
    ...
    res.status(201).json({ message: serializeMessage(message, chatKey) });
  } catch (err) {
...
router.post("/:chatId/media", requireAuth, upload.single("file"), async (req, res) => {
  try {
    ...
    res.status(201).json({ message: serializeMessage(message, null) });
  } catch (err) {
...
router.get("/:chatId", requireAuth, async (req, res) => {
  ...
  res.json({ messages: messages.reverse().map((m) => serializeMessage(m, chatKey)) });
});
...
router.get("/:chatId/poll", requireAuth, async (req, res) => {
  ...
  res.json({
    messages: messages.map((m) => serializeMessage(m, chatKey)),
...
router.patch("/:chatId/:messageId", requireAuth, async (req, res) => {
  try {
    ...
    res.json({ message: serializeMessage(message, chatKey) });
  } catch (err) {
